class NAdam(Optimizer):
    def __init__(self, params, lr=1e-03, betas=(0.9, 0.999), eps=1e-16,
                 ...
                 degenerated_to_sgd=True, fixed_decay=False, tolerance=0.5):
        if not 0.0 <= lr:
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['exp_avg_diff'] = torch.zeros_like(p.data, memory_format=torch.preserve_format) \
                        if version_higher else torch.zeros_like(p.data)
                ...
                exp_avg, exp_avg_var, exp_avg_diff = state['exp_avg'], state['exp_avg_var'], state['exp_avg_diff']
                ...
                comparison = torch.norm(exp_avg_diff) / torch.norm(grad_diff)
                if state['step'] > 1 / (1 - beta1) and (comparison > self.tolerance or comparison < 1 / self.tolerance):
                    exp_avg.mul_(beta1).add_(grad, alpha=1 - beta1)
                
                exp_avg_diff.mul_(beta1).add_(grad_diff, alpha=1 - beta1)
